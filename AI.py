...
# --- Versioning ---
# v3.2 (Advanced Move Ordering) + NMP
# - Implemented a strict move ordering bonus hierarchy to ensure tactically
#   important moves are always searched before quiet moves.
# - Replaced the old scoring system with a more robust one inside order_moves.
# - NPM implemented, gives a roughly 10% performance improvement!
...
class ChessBot:
    ...
    # --- MOVE ORDERING BONUSES (Strict Hierarchy) ---
    BONUS_PV_MOVE = 2_000_000          # The best move from a previous search (from TT)
    BONUS_GOOD_CAPTURE = 1_500_000     # Captures where the victim is worth more than the attacker
    BONUS_PROMOTION = 1_200_000        # Pawn promotions
    BONUS_KILLER_1 = 900_000           # Primary killer move for this ply
    BONUS_KILLER_2 = 850_000           # Secondary killer move
    BONUS_LOSING_CAPTURE = 300_000     # Captures where the attacker is worth more
    # History heuristic scores will fill the gap between 0 and ~100,000

    # --- NULL MOVE PRUNING (NMP) Constants ---
    NMP_DEPTH_THRESHOLD = 3  # Only apply NMP at depth 3 or greater
    NMP_REDUCTION = 3        # The standard depth reduction for the null move search
    ...
    def evaluate_board(self, board, current_turn):
        ...
        # Use the new refactored method to determine the game phase
    ...
    def is_endgame(self, board):
        """Checks if the game is in an endgame phase based on material."""
        total_material = 0
        for r in range(ROWS):
            for c in range(COLS):
                p = board[r][c]
                # Exclude kings from the material count for this check
    ...
    def order_moves(self, board, moves, moving_player_color, ply_searched=0, hash_move=None):
        ...
        for move in moves:
            ...
            # 1. PV/TT Move: Highest priority
            if move == hash_move:
                score = self.BONUS_PV_MOVE
                move_scores[move] = score
                continue

            # 2. Tactical Moves (Captures, Promotions, Special Abilities)
            if is_move_tactical(board, move):
                if isinstance(moving_piece, Pawn) and (end_pos[0] == 0 or end_pos[0] == ROWS - 1):
                    ...
                    # For Jungle Chess special moves (Knight evap, Rook path clear)
                    score = self.BONUS_LOSING_CAPTURE + 300 
            
            # 3. Quiet Moves (Killers and History Heuristic)
            else:
                if move == killers[0]:
    ...
    def negamax(self, board, depth, alpha, beta, turn_multiplier, ply):
        ...
        if (not is_in_check_now and
                depth >= self.NMP_DEPTH_THRESHOLD and
                not self.is_endgame(board)): # Cleanly calls our new method
