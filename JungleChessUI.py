# JungleChessUI.py (v4.0 - Multiprocessing & UX Overhaul)
# - Migrated AI to a non-blocking multiprocessing architecture for a fully responsive UI.
# - Redesigned the sidebar layout for better readability and usability.
# - Fixed numerous bugs related to game state, AI series, and randomness.
...
class EnhancedChessApp:
    ...
    def build_ui(self):
        ...
        self.board_image_id = self.canvas.create_image(0, 0, image=self.board_image_white, anchor='nw', tags="board")
        self.canvas.pack()
    ...
    def _build_control_widgets(self, parent_frame):
        ...
        self.bot_depth_slider.set(3)
        self.bot_depth_slider.pack(fill=tk.X, pady=(0,2))
        
        self.instant_move = tk.BooleanVar(value=False)
        ttk.Checkbutton(controls_frame, text="Instant Moves", variable=self.instant_move, style='Custom.TCheckbutton').pack(anchor=tk.W, pady=(2,2))
    ...
    def _build_status_widgets(self, parent_frame):
        self.turn_frame = ttk.Frame(parent_frame, style='Left.TFrame')
        self.turn_frame.pack(fill=tk.X, pady=(8,0))
        self.turn_label = ttk.Label(self.turn_frame, text="WHITE'S TURN", style='Status.TLabel')
        self.turn_label.pack(fill=tk.X)
        
        self.eval_frame = ttk.Frame(parent_frame, style='Left.TFrame')
        self.eval_frame.pack(fill=tk.X, pady=(9, 4))
        self.eval_score_label = ttk.Label(self.eval_frame, text="Even", style='Status.TLabel', anchor="center")
        self.eval_score_label.pack(pady=(6,4))
        self.eval_bar_canvas = tk.Canvas(self.eval_frame, height=26, bg=self.COLORS['bg_light'], highlightthickness=0)
    ...
    def _build_scoreboard_and_labels(self):
        ...
        self.top_bot_label = ttk.Label(self.right_panel, text="", font=("Helvetica", 12), background=self.COLORS['bg_medium'], foreground=self.COLORS['text_light'])
        self.top_bot_label.place(relx=0.5, rely=0.02, anchor='n')
        self.bottom_bot_label = ttk.Label(self.right_panel, text="", font=("Helvetica", 12), background=self.COLORS['bg_medium'], foreground=self.COLORS['text_light'])
        self.bottom_bot_label.place(relx=0.5, rely=0.98, anchor='s')

    def toggle_analysis_mode(self):
        # This function is now silent and only acts if the mode is correct.
        ...
        try:
            while not self.comm_queue.empty():
                ...
                if msg_type == 'log':
                    ...
                    eval_score, depth = payload
                    self.draw_eval_bar(eval_score, depth)
                elif msg_type == 'move':
                    the_move = payload[0]
                    if the_move:
    ...
    def _stop_ai_process(self):
        if self.ai_process and self.ai_process.is_alive():
            print("UI: Sending cancellation signal to AI process...")
            self.ai_cancellation_event.set()
            self.ai_process.join(timeout=0.5)
            if self.ai_process.is_alive():
                print("UI: AI process did not terminate gracefully, forcing it.")
        ...
        is_hvh_mode = self.game_mode.get() == GameMode.HUMAN_VS_HUMAN.value
        if is_hvh_mode and not self.analysis_mode_var.get():
    ...
    def _make_game_ai_move(self):
        ...
        if mode == GameMode.AI_VS_AI.value:
            if self.turn == 'white':
                is_op_playing = (self.white_playing_bot_type == "op")
                bot_class = OpponentAI if is_op_playing else ChessBot
                bot_name = self.OPPONENT_AI_NAME if is_op_playing else self.MAIN_AI_NAME
            else:
                is_op_playing = (self.white_playing_bot_type != "op")
                bot_class = OpponentAI if is_op_playing else ChessBot
                bot_name = self.OPPONENT_AI_NAME if is_op_playing else self.MAIN_AI_NAME
    ...
    def _start_analysis_if_needed(self):
        self._stop_ai_process()
        is_analysis_mode = self.analysis_mode_var.get()
        is_hvh_mode = self.game_mode.get() == GameMode.HUMAN_VS_HUMAN.value
        if is_analysis_mode and is_hvh_mode and not self.game_over:
            print("UI: Starting background analysis process...")
    ...
    def update_bot_labels(self):
        ...
        if mode == GameMode.HUMAN_VS_BOT.value:
            ...
            white_name = self.OPPONENT_AI_NAME if self.white_playing_bot_type == "op" else self.MAIN_AI_NAME
            black_name = self.MAIN_AI_NAME if self.white_playing_bot_type == "op" else self.OPPONENT_AI_NAME
        ...
        if self.board_orientation == "white":
            self.bottom_bot_label.config(text=white_label)
            self.top_bot_label.config(text=black_label)
        else:
            self.bottom_bot_label.config(text=black_label)
            self.top_bot_label.config(text=white_label)
    ...
    def board_to_canvas(self, r, c):
        if self.board_orientation == "black":
            x = (COLS - 1 - c) * SQUARE_SIZE
            y = (ROWS - 1 - r) * SQUARE_SIZE
        else:
            x = c * SQUARE_SIZE
            y = r * SQUARE_SIZE
        return x, y
    ...
    def canvas_to_board(self, x, y):
        if self.board_orientation == "black":
            c = (COLS - 1) - (x // SQUARE_SIZE)
            r = (ROWS - 1) - (y // SQUARE_SIZE)
        else:
            c = x // SQUARE_SIZE
            r = y // SQUARE_SIZE
    ...
    def process_ai_series_result(self):
        ...
        if winner_color:
            is_main_ai_win = (winner_color == 'white' and self.white_playing_bot_type == 'main') or (winner_color == 'black' and self.white_playing_bot_type == 'op')
            if is_main_ai_win:
