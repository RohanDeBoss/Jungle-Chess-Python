# TablebaseGenerator.py (v13.2 - Progress feedback, per-stage timing, bug fixes)
#
# Changes from v13.1:
#   FIX-1:  Generator5 BTM reverse link: trans[3] -> trans[4] for wtm_to_btm
#            (trans[3] is known_wins; child_flats is at trans[4])
#   FIX-2:  Generator5Vs sz_mb formula: *2 -> *1 (int8 = 1 byte, not 2)
#   NEW-1:  Progress feedback added to Generator, Generator4, Generator4Vs, Generator5 map loops
#   NEW-2:  Per-stage timing printed after each stage in every generator
#   NEW-3:  Running elapsed time shown in all progress lines
#   NEW-4:  Total time printed at end of each generator in both s and minutes
#   NEW-5:  Overall run time printed at end of __main__
...
def _fmt_elapsed(seconds):
    """Format elapsed seconds as e.g. '1m 23.4s' or '45.2s'."""
    if seconds >= 60:
        m = int(seconds // 60)
        s = seconds - m * 60
        return f"{m}m {s:.1f}s"
...
def _run_bfs_retrograde(table, unsolved_flats, transitions, trans_lookup, btm_to_wtm, wtm_to_btm, candidate_states, start_time):
    ...
    elapsed_so_far = bfs_start - start_time
    print(f"[Stage 3] BFS start | Terminal (DTM 1): {solved_count:,} | "
          f"Elapsed so far: {_fmt_elapsed(elapsed_so_far)}", flush=True)
...
class Generator:
    ...
    def generate(self):
        ...
        s1 = time.time()
        print(f"[Stage 1] Enumerating candidates...", flush=True)
        raw_candidates = []
        ...
        s2 = time.time()
        print(f"[Stage 2] Building transition cache ({total:,} states)...", flush=True)
        ...
        with ProcessPoolExecutor(max_workers=self.transition_workers,
            ...
            for done, result in enumerate(ex.map(_build_transition_worker, raw_candidates, chunksize=1024), 1):
                ...
                if done % 10_000 == 0:
                    ...
                    speed = done / max(0.001, time.time() - s2)
...
def _w5_ext_dtm(board, turn_idx):
    ...
    if nw == 2:
        ...
        key = f"{names[0]}_{names[1]}"; tb = _W5_4MAN_TABLES.get(key)
        if tb is None: return None
            ...
            for done, result in enumerate(ex.map(_build_transition_worker_5, all_flats, chunksize=2048), 1):
                if result is not None:
                    ...
                    if (flat & 1) == 0:
                        ...
                        if not trans[2]:
                            # FIX: trans[3]=known_wins, trans[4]=child_flats
                            for cflat in trans[4]: wtm_to_btm[cflat].append(flat)
                if done % 100_000 == 0:
                    elapsed = time.time() - start_time
                    speed = done / max(0.001, time.time() - s2)
...
class Generator5Vs:
    ...
    def generate(self):
        if os.path.exists(self.filename): return
        start_time = time.time()
        # FIX-2: Changed formula *2 *2 to *2 *1 since int8 is 1 byte
            ...
            for done, result in enumerate(ex.map(_build_transition_worker_5vs, all_flats, chunksize=2048), 1):
                if result is not None:
                    ...
                    if (flat & 1) == 0:
                        ...
                        if not trans[2]:
                            # FIX-1: BTM child_flats is at index 4 (index 3 is known_wins)
                            for cflat in trans[4]: wtm_to_btm[cflat].append(flat)
                
                # Progress and timing printout
                if done % 100_000 == 0:
                    elapsed = time.time() - start_time
                    speed = done / max(0.001, time.time() - s2)
                    print(f"  > {done/total*100:.1f}% ({done:,}/{total:,}) | {speed:,.0f} st/s | Elapsed: {_fmt_elapsed(elapsed)}", end='\r', flush=True)
...
if __name__ == "__main__":
    ...
    print("=== Tablebase Generator (v13.2 - The Ultimate 5-Man Generator) ===")
    
    # NEW-5: Track overall generation process timing
