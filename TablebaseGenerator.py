...
# Canonical order must match the Engine's TablebaseManager
...
def _run_bfs_retrograde(table, unsolved_flats, transitions, trans_lookup, btm_to_wtm, wtm_to_btm, candidate_states):
    ...
    for flat, trans in zip(unsolved_flats, transitions):
        ...
        if t_idx == 0:  # WTM
            ...
            if trans[1] == 0:  
                ...
                known_wins = trans[3] if len(trans) == 5 else array('H')
    ...
    while work_set or pending_work:
        ...
        print(f"  [DTM {iteration:^3}] New: {changed:<8,} | Total: {solved_count:,}/{candidate_states:,} "
              f"| Next Q: {len(work_set):<7,} | Held: {pending_count:<6,} | Time: {time.time()-iter_start:.1f}s", flush=True)
        iteration += 1
...
class Generator:
    ...
    def generate(self):
        ...
        max_dtm, decisive = _run_bfs_retrograde(self.table, unsolved_flats, transitions, trans_lookup, btm_to_wtm, wtm_to_btm, candidate_states)
        ...
        print(f"SUCCESS: {self.filename}", flush=True)
...
def _init_transition_worker_4vs(w_name, b_name):
    ...
    _W4V_HAS_PAWN = ("Pawn" in {w_name, b_name})
    _W4V_3MAN_TABLES.clear(); _W4V_PROMO_TABLES.clear()
...
def _white_win_dtm_3man(piece_name, wk, p, bk, turn_idx, is_white_piece):
    ...
    if is_white_piece:
        ...
        val = int(tb.flat[q_idx])
        if val == 0: return None
        b_wins = (t2==0 and val>0) or (t2==1 and val<0)
...
def _white_win_dtm_promo_vs(w_name, b_name, wk, wp, bk, bp, turn_idx):
    ...
    if w_name > b_name:
        ...
        q_idx = _canonical_flat_4vs(bk_f[0]*8+bk_f[1], bp_f[0]*8+bp_f[1],
                                     wk_f[0]*8+wk_f[1], wp_f[0]*8+wp_f[1], t2, hp)
        val = int(tb.flat[q_idx])
        if val == 0: return None
        b_wins = (t2==0 and val>0) or (t2==1 and val<0)
        return None if b_wins else abs(val)
    else:
        q_idx = _canonical_flat_4vs(wk[0]*8+wk[1], wp[0]*8+wp[1],
                                     bk[0]*8+bk[1], bp[0]*8+bp[1], turn_idx, hp)
        val = int(tb.flat[q_idx])
        if val == 0: return None
        return abs(val) if ((turn_idx==0 and val>0) or (turn_idx==1 and val<0)) else None
...
def _build_transition_worker_4vs(flat):
    ...
    turn_is_white = (flat % 2 == 0); opp_turn_idx = 1 - (flat % 2)
    wk = (wk_sq//8,wk_sq%8); wp = (wp_sq//8,wp_sq%8)
    bk = (bk_sq//8,bk_sq%8); bp = (bp_sq//8,bp_sq%8)
            ...
            if ext == _IN_TABLE_SENTINEL:
                ...
                if w_p and b_p:
                    child_flats.append(_canonical_flat_4vs(
                        board.white_king_pos[0]*8+board.white_king_pos[1], w_p.pos[0]*8+w_p.pos[1],
                        board.black_king_pos[0]*8+board.black_king_pos[1], b_p.pos[0]*8+b_p.pos[1],
                        opp_turn_idx, _W4V_HAS_PAWN))
            elif ext is not None and ext > 0:
                known_wins.append(ext + 1)
            board.unmake_move(record)
        return (flat, ('w', immediate_win, array('H', known_wins), array('I', child_flats)))
...
# ==============================================================================
_W5_HAS_PAWN = False
_W5_P1_NAME = None; _W5_P2_NAME = None; _W5_P3_NAME = None
...
def _init_transition_worker_5(p1_name, p2_name, p3_name):
    ...
    _W5_WK_OBJ = King('white')
    _W5_P1_OBJ = PIECE_CLASS_BY_NAME[p1_name]('white')
    _W5_P2_OBJ = PIECE_CLASS_BY_NAME[p2_name]('white')
    _W5_P3_OBJ = PIECE_CLASS_BY_NAME[p3_name]('white')
    _W5_BK_OBJ = King('black')
    pc = _W5_BOARD.piece_counts
    pc['white'][King] = pc['black'][King] = 1
    pc['white'][type(_W5_P1_OBJ)] += 1
    pc['white'][type(_W5_P2_OBJ)] += 1
    pc['white'][type(_W5_P3_OBJ)] += 1
...
def _w5_ext_dtm(board, turn_idx):
    ...
    if nw == 2:
        names = sorted([type(p).__name__ for p in w_nk])
        key   = f"{names[0]}_{names[1]}"
        ...
        hp    = ("Pawn" in {n1, n2})
        q_idx = _canonical_flat_4(wkp[0]*8+wkp[1],
                                   ws[0].pos[0]*8+ws[0].pos[1], ws[1].pos[0]*8+ws[1].pos[1],
                                   bkp[0]*8+bkp[1], turn_idx, hp, n1==n2)
        val   = int(tb.flat[q_idx])
        if val == 0: return None
        return abs(val) if ((turn_idx==0 and val>0) or (turn_idx==1 and val<0)) else None

    if nw == 1:
        wn  = type(w_nk[0]).__name__
        tb  = _W5_3MAN_TABLES.get(wn)
        if tb is None: return None
        q_idx = _canonical_flat_3(wkp[0]*8+wkp[1], w_nk[0].pos[0]*8+w_nk[0].pos[1],
                                   bkp[0]*8+bkp[1], turn_idx, wn=="Pawn")
        val   = int(tb.flat[q_idx])
        if val == 0: return None
        return abs(val) if ((turn_idx==0 and val>0) or (turn_idx==1 and val<0)) else None
...
def _build_transition_worker_5(flat):
    rest   = flat // 2
    bk_sq  = rest % 64; rest //= 64
    p3_sq  = rest % 64; rest //= 64
    p2_sq  = rest % 64; rest //= 64
    p1_sq  = rest % 64; wk_idx = rest // 64
    wk_sq  = PAWN_WK_SQUARES[wk_idx] if _W5_HAS_PAWN else NON_PAWN_WK_SQUARES[wk_idx]
    turn_is_white = (flat % 2 == 0); opp_turn_idx = 1 - (flat % 2)
    wk=(wk_sq//8,wk_sq%8); p1=(p1_sq//8,p1_sq%8); p2=(p2_sq//8,p2_sq%8)
    p3=(p3_sq//8,p3_sq%8); bk=(bk_sq//8,bk_sq%8)

    board = _W5_BOARD; g = board.grid
    for p in (_W5_WK_OBJ,_W5_P1_OBJ,_W5_P2_OBJ,_W5_P3_OBJ,_W5_BK_OBJ):
        if p.pos: g[p.pos[0]][p.pos[1]] = None
    _W5_WK_OBJ.pos,_W5_P1_OBJ.pos,_W5_P2_OBJ.pos = wk,p1,p2
    _W5_P3_OBJ.pos,_W5_BK_OBJ.pos = p3,bk
    _W5_WK_OBJ._list_pos,_W5_P1_OBJ._list_pos,_W5_P2_OBJ._list_pos = 0,1,2
    _W5_P3_OBJ._list_pos,_W5_BK_OBJ._list_pos = 3,0
    g[wk[0]][wk[1]],g[p1[0]][p1[1]],g[p2[0]][p2[1]] = _W5_WK_OBJ,_W5_P1_OBJ,_W5_P2_OBJ
    g[p3[0]][p3[1]],g[bk[0]][bk[1]] = _W5_P3_OBJ,_W5_BK_OBJ
    board.white_king_pos,board.black_king_pos = wk,bk
    board.white_pieces[:] = [_W5_WK_OBJ,_W5_P1_OBJ,_W5_P2_OBJ,_W5_P3_OBJ]
    board.black_pieces[:] = [_W5_BK_OBJ]
        ...
        for start, end in moves:
            ...
            if len(w_nk) == 3:
                ...
                if cn1==_W5_P1_NAME and cn2==_W5_P2_NAME and cn3==_W5_P3_NAME:
                    child_flats.append(_canonical_flat_5(
                        wkp[0]*8+wkp[1],
                        ws[0].pos[0]*8+ws[0].pos[1], ws[1].pos[0]*8+ws[1].pos[1],
                        ws[2].pos[0]*8+ws[2].pos[1], bkp[0]*8+bkp[1],
                        opp_turn_idx, _W5_HAS_PAWN, cn1, cn2, cn3))
                else:
                    key = (cn1, cn2, cn3)
                    if key in _W5_PROMO_TABLES:
                        tb = _W5_PROMO_TABLES[key]
                        hp = ("Pawn" in key)
                        q_idx = _canonical_flat_5(
                            wkp[0]*8+wkp[1],
                            ws[0].pos[0]*8+ws[0].pos[1], ws[1].pos[0]*8+ws[1].pos[1],
                            ws[2].pos[0]*8+ws[2].pos[1], bkp[0]*8+bkp[1],
                            opp_turn_idx, hp, cn1, cn2, cn3)
                        val = int(tb.flat[q_idx])
                        if val < 0: known_wins.append(abs(val) + 1)
            else:
...
def _gen_valid_5same_indices_numpy(total, p1n, p2n, p3n, has_pawn, chunk_size=2_000_000):
    ...
    for start in range(0, total, chunk_size):
        end_c  = min(start + chunk_size, total)
        flat   = np.arange(start, end_c, dtype=np.int64)
        rest   = flat // 2
        bk_arr = rest % 64; rest //= 64
        p3_arr = rest % 64; rest //= 64
        p2_arr = rest % 64; rest //= 64
        p1_arr = rest % 64; wk_idx = rest // 64
        wk_arr = wk_list[wk_idx]
...
class Generator5:
    def __init__(self, p1_class, p2_class, p3_class):
        ps = sorted([p1_class, p2_class, p3_class],
    ...
    def generate(self):
        ...
        with ProcessPoolExecutor(max_workers=self.transition_workers,
                                 initializer=_init_transition_worker_5,
                                 initargs=(self.p1_name, self.p2_name, self.p3_name)) as ex:
            for done, result in enumerate(ex.map(_build_transition_worker_5, all_flats, chunksize=2048), 1):
                if result is not None:
                    flat, trans = result
                    unsolved_flats.append(flat); transitions.append(trans); trans_lookup[flat] = trans
                    if (flat & 1) == 0:
...
# ==============================================================================
_W5V_HAS_PAWN = False
_W5V_W1_NAME = None; _W5V_W2_NAME = None; _W5V_B_NAME = None
...
def _init_transition_worker_5vs(w1_name, w2_name, b_name):
    ...
    pc = _W5V_BOARD.piece_counts
    pc['white'][King] = pc['black'][King] = 1
    pc['white'][type(_W5V_WP1_OBJ)] += 1; pc['white'][type(_W5V_WP2_OBJ)] += 1
    pc['black'][type(_W5V_BP_OBJ)]  += 1
...
def _w5v_ext_dtm(board, turn_idx):
    ...
    if nw == 2 and nb == 1:
        ...
        q_idx = _canonical_flat_5vs(wkp[0]*8+wkp[1],
                                    ws[0].pos[0]*8+ws[0].pos[1], ws[1].pos[0]*8+ws[1].pos[1],
    ...
    if nw == 2 and nb == 0:
        names = sorted([type(p).__name__ for p in w_nk])
        tb    = _W5V_4MAN_TABLES.get(f"{names[0]}_{names[1]}")
        if tb is None: return None
        ws    = sorted(w_nk, key=lambda p: (_PIECE_CANONICAL_ORDER.get(type(p),99), p.pos[0]*8+p.pos[1]))
        n1,n2 = type(ws[0]).__name__, type(ws[1]).__name__
        hp    = ("Pawn" in {n1, n2})
    ...
    if nw == 1 and nb == 1:
        wn, bn = type(w_nk[0]).__name__, type(b_nk[0]).__name__
        names  = sorted([wn, bn]); sn1,sn2 = names[0], names[1]
        tb     = _W5V_4VS_TABLES.get(f"{sn1}_vs_{sn2}")
        if tb is None: return None
        hp = ("Pawn" in {wn, bn})
        if wn <= bn:
            q_idx = _canonical_flat_4vs(wkp[0]*8+wkp[1], w_nk[0].pos[0]*8+w_nk[0].pos[1],
                                         bkp[0]*8+bkp[1], b_nk[0].pos[0]*8+b_nk[0].pos[1], turn_idx, hp)
            val   = int(tb.flat[q_idx])
            if val == 0: return None
            return abs(val) if ((turn_idx==0 and val>0) or (turn_idx==1 and val<0)) else None
        else:
...
def _build_transition_worker_5vs(flat):
    ...
    bp_sq  = rest % 64; rest //= 64
    bk_sq  = rest % 64; rest //= 64
    wp2_sq = rest % 64; rest //= 64
    wp1_sq = rest % 64; wk_idx = rest // 64
    wk_sq  = PAWN_WK_SQUARES[wk_idx] if _W5V_HAS_PAWN else NON_PAWN_WK_SQUARES[wk_idx]
    turn_is_white = (flat % 2 == 0); opp_turn_idx = 1 - (flat % 2)
    wk=(wk_sq//8,wk_sq%8); wp1=(wp1_sq//8,wp1_sq%8); wp2=(wp2_sq//8,wp2_sq%8)
    bk=(bk_sq//8,bk_sq%8); bp=(bp_sq//8,bp_sq%8)

    board = _W5V_BOARD; g = board.grid
    for p in (_W5V_WK_OBJ,_W5V_WP1_OBJ,_W5V_WP2_OBJ,_W5V_BK_OBJ,_W5V_BP_OBJ):
        if p.pos: g[p.pos[0]][p.pos[1]] = None
    _W5V_WK_OBJ.pos,_W5V_WP1_OBJ.pos,_W5V_WP2_OBJ.pos = wk,wp1,wp2
    _W5V_BK_OBJ.pos,_W5V_BP_OBJ.pos = bk,bp
    _W5V_WK_OBJ._list_pos,_W5V_WP1_OBJ._list_pos,_W5V_WP2_OBJ._list_pos = 0,1,2
    _W5V_BK_OBJ._list_pos,_W5V_BP_OBJ._list_pos = 0,1
    ...
    board.white_king_pos,board.black_king_pos = wk,bk
    board.white_pieces[:] = [_W5V_WK_OBJ,_W5V_WP1_OBJ,_W5V_WP2_OBJ]
    board.black_pieces[:] = [_W5V_BK_OBJ,_W5V_BP_OBJ]
        ...
        for start, end in moves:
            ...
            if ext == _IN_TABLE_SENTINEL:
                ...
                child_flats.append(_canonical_flat_5vs(
                    ...
                    ws[0].pos[0]*8+ws[0].pos[1], ws[1].pos[0]*8+ws[1].pos[1],
                    board.black_king_pos[0]*8+board.black_king_pos[1], b_nk[0].pos[0]*8+b_nk[0].pos[1],
                    opp_turn_idx, _W5V_HAS_PAWN, type(ws[0]).__name__, type(ws[1]).__name__))
...
def _gen_valid_5vs_indices_numpy(total, w1n, w2n, bn, has_pawn, same_wp, chunk_size=2_000_000):
    ...
    for start in range(0, total, chunk_size):
        ...
        bp_arr = rest % 64;  rest //= 64
        bk_arr = rest % 64;  rest //= 64
        wp2_arr = rest % 64; rest //= 64
        wp1_arr = rest % 64; wk_idx = rest // 64
        wk_arr  = wk_list[wk_idx]
...
class Generator5Vs:
    def __init__(self, w1_class, w2_class, b_class):
        ws = sorted([w1_class, w2_class],
                    key=lambda c: (_PIECE_CANONICAL_ORDER.get(c,99), c.__name__))
        self.w1_name, self.w2_name = ws[0].__name__, ws[1].__name__
    ...
    def generate(self):
        ...
        start_time = time.time()
        sz_mb = self.wk_size * 64**4 * 2 * 2 / 1024**2
        print(f"\n{'='*60}\n GENERATING: K+{self.w1_name}+{self.w2_name} vs K+{self.b_name}", flush=True)
        print(f" Estimated file size: {sz_mb:.0f} MB\n{'='*60}", flush=True)
        ...
        with ProcessPoolExecutor(max_workers=self.transition_workers,
                                 initializer=_init_transition_worker_5vs,
                                 initargs=(self.w1_name, self.w2_name, self.b_name)) as ex:
            for done, result in enumerate(ex.map(_build_transition_worker_5vs, all_flats, chunksize=2048), 1):
                if result is not None:
                    flat, trans = result
                    unsolved_flats.append(flat); transitions.append(trans); trans_lookup[flat] = trans
                    if (flat & 1) == 0:
                        ...
                        if not trans[2]:
                            for cflat in trans[4]: wtm_to_btm[cflat].append(flat)
                if done % 100_000 == 0:
                    speed = done / max(0.001, time.time() - s)
...
def is_possible(pieces):
    ...
    for p in pieces:
        counts[p] += 1
    # Max starting pieces: 2 Rooks, 2 Bishops, 2 Knights. 
    if counts[Rook] > 2 or counts[Bishop] > 2 or counts[Knight] > 2:
        return False
    return True

if __name__ == "__main__":
    _install_main_interrupt_ignores()
    print("=== Tablebase Generator (v13.0 - The Ultimate 5-Man Generator) ===")
    ...
    for w1 in non_pawn:
        for w2 in non_pawn:
            ...
            for b in non_pawn:
                ws  = sorted([w1.__name__, w2.__name__],
                             key=lambda n: (_PIECE_CANONICAL_ORDER.get(PIECE_CLASS_BY_NAME[n],99), n))
                key = (ws[0], ws[1], b.__name__)
                if key not in seen_5vs:
                    seen_5vs.add(key)
                    Generator5Vs(PIECE_CLASS_BY_NAME[ws[0]], PIECE_CLASS_BY_NAME[ws[1]], b).generate()
